...
router.post('/login', csrfProtection, asyncHandler( async function(req, res, next) {
  ...
  if (thisUser) {
    ...
    const passwordMatch = await bcrypt.compare(password, thisUser.hashedPassword.toString())
    ...
    if (passwordMatch) {
      ...
      res.render('login', {username, email, csrfToken: req.csrfToken()})
